...
class split:
    ...
    def dataSplit(self, image_files, highlow_json_data, high_json, low_json, dataset):
        high_file_name = 0
        low_file_name = 0
        for file_name in image_files:
            ...
            if(algorithm.even(file_name)):
                ...
                save_image_path = f"{self.high_path}/{dataset}/image/{high_file_name}.png"

                # image 저장
                io.image_io.imageSave(save_jitter_path, io.image_io.imageLoad(images["path"])) # jitter
                io.image_io.imageSave(save_image_path, io.image_io.imageLoad(images["path"].replace("jitter_", ""))) # 원본 image
                ...
                save_image_path = f"{self.low_path}/{dataset}/image/{low_file_name}.png"
    ...
    def run(self):
        for dataset in self.dataset_setting:
            ...
            image_files = io.files_io.filesLoad(f"{self.highlow_path}/{dataset}/jitter_image")

            highlow_json_data = io.json_io.jsonLoad(f"{self.highlow_path}/{dataset}/json/data.json")
            high_json, low_json = self.dataSplit(image_files, highlow_json_data, high_json, low_json, dataset)
