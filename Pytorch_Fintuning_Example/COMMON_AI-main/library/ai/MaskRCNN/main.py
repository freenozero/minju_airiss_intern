from library.ai.MaskRCNN.utils.header import *
from library.ai.MaskRCNN.utils.io import *

from library.ai.MaskRCNN.utils.log import logger
from library.ai.MaskRCNN.utils.view import *

from library.ai.MaskRCNN.config import train_configs as tr_cfg
from library.ai.MaskRCNN.config import predict_configs as pr_cfg

from library.ai.MaskRCNN.dataset.dataset import COCODataset
...
class DataLoader:
    ...
    def build(self):
        return torch.utils.data.DataLoader(
            COCODataset(
                ...
                coco=self.json_data,
                transforms=self._get_transform(is_transform=self.is_transform),
            ),
    ...
    def _get_transform(self, is_transform):
        transforms = []
...
class Train:
    ...
    def start(self):
        ...
        for epoch in range(self.epochs):
            ...
            #todo : categories 별 식별율 로그 추가
            
            #
        
        logger.info("Train End")

    #TODO : working restart
    # def restart(self):
        
    #     logger.info("Train ReStart")
    ...
    #     #optimizer 설정
    #     optimizer = Optimizer()
    #     optm, lr_sch = optimizer.build(model_params=mdl.params())
    #     # optm.load_state_dict(checkpoint["optimizer_state_dict"])
        
    #     # self.epochs = checkpoint["epoch"]
        
    #     for epoch in range(self.epochs):
    #         train_one_epoch(mdl, optm, train_dl, device, epoch, print_freq=10)
            
    #         torch.save(mdl.state_dict(), f"{self.save_model_path}/epoch_{epoch}.pth" )
    #         torch.save(mdl.state_dict(), f"{self.save_model_path}/lastest.pth" )
            
    #         lr_sch.step()
            
    #         #todo : categories 별 식별율 로그 추가
            
    #         evaluate(mdl, val_dl, device=device)
